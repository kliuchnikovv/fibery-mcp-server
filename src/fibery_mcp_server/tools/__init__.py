...
from fibery_mcp_server.tools.search import search_tool_name, search_tool, handle_search
...
def handle_list_tools():
    return [current_date_tool(), schema_tool(), database_tool(), query_tool(), search_tool(), create_entity_tool(), create_entities_batch_tool(), update_entity_tool()]
...
async def handle_tool_call(fibery_client: FiberyClient, name: str, arguments: Dict[str, Any]):
    if name == schema_tool_name:
        ...
        return await handle_search(fibery_client, arguments)
